...
ggplot(overall_mean, aes(x = month, y = Mean_Value, color = Measurement, group = Measurement)) +
    ...
    geom_point(aes(shape = Measurement)) +
    labs(
        title = "Overall Mean Growth Pattern (Length, Width, Weight) Over Time",
        x = "Month",
        y = "Mean Value",
        color = "Measurement"
    ) +
